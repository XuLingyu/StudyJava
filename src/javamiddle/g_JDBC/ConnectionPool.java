...
//创建连接和关闭连接的过程也是比较!!!消耗时间!!!的，当多线程并发的时候，系统就会变得很!!!卡顿!!!。
//同时，一个数据库同时支持的!!!连接总数也是有限!!!的，如果多线程并发量很大，
// 那么数据库连接的总数就会被消耗光，后续线程发起的!!!!数据库连接就会失败!!!。

//数据库连接池原理-使用池
//1. 连接池在使用之前，就会创建好一定数量的连接。任何线程需要使用连接，那么就从连接池里面借用，而不是自己重新创建.
//   使用完毕后，又把这个连接归还给连接池供下一次或者其他线程使用
//2. 倘若发生多线程并发情况，连接池里的连接被借用光了，那么其他线程就会临时等待，直到有连接被归还回来，再继续使用。
//   整个过程，这些连接都不会被关闭，而是不断的被循环使用，从而节约了启动和关闭连接的时间。


//ConnectionPool:
//1. ConnectionPool() 构造方法约定了这个连接池一共有多少连接
//2. 在init() 初始化方法中，创建了size条连接。 注意，这里不能使用try-with-resource
...
public class ConnectionPool {
    ...
    //使用完毕后，归还这个连接到连接池，并且在归还完毕后，调用notifyAll，通知那些等待的线程，有新的连接可以借用了
